...
contains
	...
	! Subroutine to write data to disk. Called intermittently, determined by save_step supplied at run-time
        ...
        call MPI_IALLGATHER(nvirt_loc,1,MPI_INTEGER,nvirt_glob,1,MPI_INTEGER,MPI_COMM_WORLD,request(2),ierr)
		call MPI_IALLGATHER(nhalo_loc,1,MPI_INTEGER,nhalo_glob,1,MPI_INTEGER,MPI_COMM_WORLD,request(3),ierr)
        ...
        posrange(:) = ntotal_loc + [1,nvirt_loc]
        ...
        posrange(:) = ntotal_loc + nvirt_loc + [1,nhalo_loc]
        ...
        posrange(:) = ntotal_loc + nvirt_loc + nhalo_loc + [1,nghos_loc]
	...
	! Subroutine for writing initial configuration data using MPI IO subroutines
		...
		implicit none
		integer:: ntotal_glob(numprocs),nvirt_glob(numprocs),posrange(2)
        ...
        global_dims(:) = [dim,nvirt]
        displ(:) = [0,SUM(nvirt_glob(1:procid))]
