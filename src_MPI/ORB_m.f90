module ORB_m
	...
	use globvar,		only: scale_k,parts,ntotal_loc,nvirt_loc
	use globvar_para,	only: procid,numprocs,ierr,MPI_ftype,repartition_mode,node_cax,node_cut
	use mpi
	use param,			only: f,dim,hsml
	
	public:: ORB
	private:: particle_grid,ORB_bounds,P_trim,data_to_next_node,ORB_boundingbox,potential_neighbour_process_search,subdomain_neighbour
	
contains	
	!==============================================================================================================================
	subroutine ORB
	! Container subroutine for the bulk of the ORB algorithm, including the initial exchange of physical and halo particles
		use globvar,		only: itimestep,ntotal,nvirt,nhalo_loc,t_graph,t_dist
		...
		integer:: d,i,j,k,ngridx_real(dim),ngridx_all(dim),nphys_recv_all,request_phys(4*numprocs),request_halo(4*numprocs),physsearchrange_ini(2),&
            virtsearchrange_ini(2),n_request,status(MPI_STATUS_SIZE,4*numprocs),nphys_send_all,procrange_ini(2),tree_layers,&
            gridind_ini(dim,2),diffusedepth,nvirt_recv_all
		...
		if ( (itimestep.eq.1) .or. ((itimestep-prev_part_tstep.ge.ORBcheck1) .and. (mod(itimestep-prev_part_tstep,ORBcheck2).eq.0)) ) then
			...
			if ( (repartition_mode.gt.1) .or. (itimestep.eq.1) ) then
				...
				call particle_grid( ngridx_real,ngridx_all,dcell,mingridx_ini,maxgridx_ini )
				
				! Calculating current aspect ratio.
				do d = 1,dim
					box_ratio_current(:,d) = DBLE(ngridx_real(d))/DBLE(ngridx_real(:))
				end do
				...
				if (itimestep.ne.1) deallocate(nphys_send,nphys_recv,nvirt_send,nvirt_recv,nhalo_send,nhalo_recv,halotype_indexed,&
                    haloupdatetype_indexed)
				allocate( nphys_send(n_process_neighbour),&
						nphys_recv(n_process_neighbour),&
                        nvirt_send(n_process_neighbour),&
                        nvirt_recv(n_process_neighbour),&
		...
		diffusedepth = 0
		physsearchrange_ini(:) = (/1,ntotal_loc/)
        virtsearchrange_ini(:) = (/ntotal_loc+1,ntotal_loc+nvirt_loc/)
		i = ORB_sendrecv_diffuse( diffusedepth,physsearchrange_ini,virtsearchrange_ini,n_request,request_phys,nphys_recv_all,&
            nvirt_recv_all )
        
		! halo particle distribution
		call ORB_sendrecv_halo( request_phys,request_halo,nphys_recv_all,nvirt_recv_all,n_request )
		
		! update virtual particles
!~ 		call virt_part(.true.)
		
		if (repartition_mode.gt.1) prev_load = ntotal_loc!+nvirt_loc
		
		do i = ntotal_loc+nvirt_loc+1,ntotal_loc+nvirt_loc+nhalo_loc
			parts(i)%indloc = i
		end do
...
!~ 		parts(ntotal_loc+nvirt_loc+1:ntotal_loc+nvirt_loc+nhalo_loc)%itype = 2
		t_dist = t_dist + MPI_WTIME()
		
	end subroutine ORB
	
	!==============================================================================================================================
	subroutine particle_grid( ngridx_real,ngridx_all,dcell,mingridx_all,maxgridx_all )
	...
	! cell. Each MPI process holds a global copy of the entire grid, in preperation for ORB
		...
		integer,intent(out):: ngridx_real(3),ngridx_all(3)
		real(f),intent(out):: mingridx_all(3),maxgridx_all(3)
		integer:: i,j,k,d,n,icell,jcell,kcell,n_nonzerocells,n_nonzerocells_perprocess(numprocs),n_nonzerocells_total,pid,&
				displ(numprocs),cellmins(3),cellmaxs(3),cellrange(3),request(4),status(MPI_STATUS_SIZE,4),ngridx(dim)
		real(f):: minx_real(3)=HUGE(1._f),maxx_real(3)=-HUGE(1._f),minx_all(3),maxx_all(3),mingridx_real(3),maxgridx_real(3)
        ...
        call MPI_IALLREDUCE(minx_real,mingridx_real,3,MPI_ftype,MPI_MIN,MPI_COMM_WORLD,request(1),ierr)
        call MPI_IALLREDUCE(maxx_real,maxgridx_real,3,MPI_ftype,MPI_MAX,MPI_COMM_WORLD,request(2),ierr)
        
        minx_all(:) = minx_real(:)
        maxx_all(:) = maxx_real(:)
        do i = ntotal_loc+1,ntotal_loc+nvirt_loc
            do d = 1,dim
                minx_all(d) = MIN(minx_all(d),parts(i)%x(d))
                maxx_all(d) = MAX(maxx_all(d),parts(i)%x(d))
            end do
        end do
        
        call MPI_IALLREDUCE(minx_all,mingridx_all,3,MPI_ftype,MPI_MIN,MPI_COMM_WORLD,request(3),ierr)
        call MPI_IALLREDUCE(maxx_all,maxgridx_all,3,MPI_ftype,MPI_MAX,MPI_COMM_WORLD,request(4),ierr)
        
        call MPI_WAITALL(4,request,status,ierr)
        
        mingridx_real(:) = mingridx_real(:) - dcell
        maxgridx_real(:) = maxgridx_real(:) + dcell
        mingridx_all(:) = mingridx_all(:) - dcell
		maxgridx_all(:) = maxgridx_all(:) + dcell
		...
		ngridx_all(:) = int((maxgridx_all(:)-mingridx_all(:))/dcell) + 1
		maxgridx_all(:) = mingridx_all(:) + dcell*ngridx_all(:)
		
		! Reducing search area by locating indices in which local particles are contained within
		cellmins(:) = int((minx_all(:) - mingridx_all(:))/dcell) + 1
		cellmaxs(:) = int((maxx_all(:) - mingridx_all(:))/dcell) + 1
		cellrange(:) = cellmaxs(:) - cellmins(:) + 1
        
		! Creating list of non-zero grid cells ------------------------------------------------------------------------------------
		Plist_size = MIN(ntotal_loc,PRODUCT(cellrange(:)))
		allocate( pincell_ORB(ngridx_all(1),ngridx_all(2),ngridx_all(3)),&
		...
		do i = 1,ntotal_loc!+nvirt_loc
			icell = int((parts(i)%x(1)-mingridx_all(1))/dcell) + 1
			jcell = int((parts(i)%x(2)-mingridx_all(2))/dcell) + 1
			kcell = int((parts(i)%x(3)-mingridx_all(3))/dcell) + 1
