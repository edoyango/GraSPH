...
module weakly_compressible_particles_m
    ...
    use grasph_constants_m, only: fp, ndims, pi, ntensor_elems_voigt, ntensor_offaxis_elems
    ...
    public:: linear_eos_state_updater_t, tait_eos_state_updater_t, ghost_state_updater_t, &
...
contains
    ...
    subroutine tait_eos_update_state(self, ps, n, dt)
        ...
        integer:: i
        do i = 1, n
            ps(i)%p = self%rho_ref*ps(i)%c*ps(i)%c/real(self%gamma, kind=fp)* &
                      ((ps(i)%rho/self%rho_ref)**self%gamma - 1._fp)
        end do
    ...
    subroutine ghost_state_update(self, ps, n, dt)
        ...
        do i = 1, n
            projection(:) = dot_product(ps(i)%original%v(:), self%surface_normal(:))*self%surface_normal(:)
            ps(i)%v(:) = ps(i)%original%v(:) - 2._fp*projection(:)
            ps(i)%rho = ps(i)%original%rho
            ps(i)%mass = ps(i)%original%mass
            ps(i)%p = ps(i)%original%p
            ps(i)%c = ps(i)%original%c
        end do
    ...
    subroutine dp_visco_elastic_state_update(self, ps, n, dt)
        ...
        call linear_eos_update_state(self, ps, n, dt)

        do i = 1, n
            ! calculate second invariant of deformation rate tensor.
            mag_strain_rate = 0._fp
            do d = 1, ndims
                mag_strain_rate = mag_strain_rate + ps(i)%strain_rate(d)**2
            end do
            do d = 1, ntensor_offaxis_elems
                mag_strain_rate = mag_strain_rate + 2._fp*ps(i)%strain_rate(ndims + d)**2
            end do
            mag_strain_rate = max(sqrt(mag_strain_rate), tiny(1._fp)) ! tiny(1) to make sure non-zero

            ! viscous stress with yield criterion
            ps(i)%stress(:) = (self%cohesion + tan(self%friction_angle)*ps(i)%p)/mag_strain_rate*ps(i)%strain_rate(:)
            ! minus pressure along principal components.
            ps(i)%stress(1:ndims) = ps(i)%stress(1:ndims) - ps(i)%p
        end do
    ...
    subroutine eos_viscous_stress_ghost_state_update(self, ps, n, dt)
        ...
        do i = 1, n
            ...
            ps(i)%p = ps(i)%original%p
            ps(i)%c = ps(i)%original%c
            ps(i)%stress(:) = ps(i)%original%stress(:)
        end do
