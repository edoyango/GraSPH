...
contains
    ...
    subroutine test_particles_init()
        ...
        call check(psys%name == "test", "Particle set name not initialised to 'test'")

        ! check member values set correctly
        call check(psys%initialised(), "Particle initilization logical not set to .true.")
        ...
        call check(is_equal(psys%particles%size, 16), "Particle array size incorrect")
    ...
    subroutine test_linear_eos_wc_particles()
        ...
        call psys%init(n=5, name="test", particle_template=ps_template, state_updaters=state_updaters)

        do i = 1, 5
            psys%particles%rho(i) = real(i, kind=fp)
            psys%particles%c(i) = 2._fp
        end do
        ...
        do i = 1, 5
            ...
            class is (eos_particles_t)
                call check( &
                    is_close(ps%p(i), 4._fp*real(i - 1, kind=fp)), &
    ...
    !     type(particle_system_t):: psys
    !     type(eos_particles_t):: eos_part
    !     type(base_particles_t):: base_part
    !     integer:: i, d
    !     character:: dc
